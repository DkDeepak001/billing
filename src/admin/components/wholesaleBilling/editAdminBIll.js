...
export class editAdminBIll extends Component {
    constructor() {
        super()
        var cookieValue;
  ...
  componentDidMount() {
        ...
        axios.get('http://billing-software.dkdeepak.com/api/fetchBill.php/?Aid=' + this.props.match.params.id )
            .then((response) => {
                this.setState({billingproducts:JSON.parse(response.data.products)},()=>{console.log()})
                this.setState({billNo:response.data.id})
                this.setState({branchId:response.data.branchId})
                this.setState({company_name:response.data.company_name})
                this.setState({time: response.data.time})
                this.setState({total: response.data.total})
                this.setState({tandc: response.data.terms})
                this.setState({description: response.data.description})
                this.setState({advancePayment: response.data.advancePayment})
                this.setState({balancePayment: response.data.balancePayment})
                this.setState({pageTotal: response.data.totalPrice})
        })
        }
        ...
        updateBill = () => {
            ...
            allData.append('billNo', this.state.billNo);
            allData.append('billData', JSON.stringify(this.state.billingproducts));
            allData.append('balancePayment', (parseFloat(this.state.pageTotal).toFixed(2) - parseFloat(this.state.advancePayment).toFixed(2)));
            allData.append('pageTotal', this.state.pageTotal);
            axios({
                method: 'post',
                url: 'http://billing-software.dkdeepak.com/api/fetchBill.php/?AdminEditBill=1',
                data: allData,
        ...
        editCalculatePrice = (event) => {
            ...
            const fetchPrice = this.state.billingproducts[getIndex].billProductPrice;
            const fetchunit = this.state.billingproducts[getIndex].billUnits;
            const FName = this.state.billingproducts[getIndex].billProductName;
            const Pquantity = this.state.billingproducts[getIndex].billQuantity;
            const OrgQty = this.state.billingproducts[getIndex].billOrgQty;
            ...
            this.setState(prevState => ({
                billingproducts: prevState.billingproducts.map((x, index) =>
        ...
        <div className="b_container" id="b_container">
           ...
           <div className="Wholesale_header">
            ...
            <div className='bill_style_row'>
                            <h2 style={{ padding: "2% 0%" }}>Bill Details:</h2>
                        </div>
                        <div className='bill_style_row'>
                            <h4 style={{ padding: "2% 0%", color: "rgba(34, 34, 34, 0.815)" }}>Bill No : {this.state.billNo}</h4>
                        </div>
                        <div className='bill_style_row'>
                            <h4 style={{ padding: "2% 0%", color: "rgba(34, 34, 34, 0.815)" }}>Branch Id : {this.state.branchId}</h4>
                        </div>
                        <div className='bill_style_row'>
                            <h4 style={{ padding: "2% 0%", color: "rgba(34, 34, 34, 0.815)" }}>Company name : {this.state.company_name}</h4>
                        </div>
                        <div className='bill_style_row'>
                            <h4 style={{ padding: "2% 0%", color: "rgba(34, 34, 34, 0.815)" }}>Date &#38; time : {this.state.time}</h4>
                        </div>
            ...
            bill === null ? <></> :
                <div className='A4pageToPrint'>
                    ...
                    <div className="A4printBranchName">
                        <h4 style={{ paddingTop: "1%" }}>Branch name:{localStorage.getItem("userName")}</h4>
