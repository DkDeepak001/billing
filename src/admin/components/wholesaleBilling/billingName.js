...
import NewuserForm from '../adminCustomer/newCustomer';
...
export class billingName extends Component {
    ...
    getData = () => {
        axios.get('http://billing-software.dkdeepak.com/api/getAdminCustomer.php')
            .then(result => {
    ...
    selectData(id, company_name) {
        ...
        this.setState({ fetchData: output }, () => {
            localStorage.setItem("cid", this.state.fetchData[0].uid);
    ...
    newuser = () => {
        this.setState({ NewUser: !this.state.NewUser }, () => {
            const elec = document.getElementById("Areg_container");
            const ele = document.getElementById("Areg_background");
            if (ele !== null) {
                elec.style.margin = "0%";
    ...
    render() {
        ...
        return (
            <div className='billingName_container'>
                ...
                {localStorage.getItem('company_name') === null ? <></> : <>{this.props.history.push(`/Admin/billingName/billing/${localStorage.getItem('company_name')}`)}</>}
                {n ?
                            ...
                            <div className='billName_row'>
                                ...
                                <div className='billName_row_button' >
                                    {this.state.fetchData.length !== 1 ? <Button type='button' variant='contained' className='css-button-rounded--blue' disable onClick={this.submitName} disabled> Next</Button>
                                        : <Link to={`/Admin/billingName/billing/${this.state.selectedValues}`}>
