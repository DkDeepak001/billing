...
export class productAnalytics extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('http://billing-software.dkdeepak.com/api/fetchBalanceProduct.php/?branchId=' + this.props.match.params.branchId + '&pid=' + this.props.match.params.pid)
            .then((response) => {
            ...
            <div className='pa_analytics'>
                <div className='pa_header'>
                    <div className='pa_header_heading'>
                        <h2>Product Analytics of {this.state.ProductName} in {this.props.match.params.branchId}</h2>
                        <Link to={`/Admin/allProduct/${this.props.match.params.pid}`}><button className='css-button-rounded--green'>View overall statistics</button></Link>
                    </div>
